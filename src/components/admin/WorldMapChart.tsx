...
export default function WorldMapChart({ data, style, isDark = true }: WorldMapChartProps) {
  ...
  const [panelWidth, setPanelWidth] = useState(23.333); // percentage (30% less than default 33.333%)
      ...
      {!isPanelCollapsed && (
        <div
          ...
          className={`relative w-1 cursor-col-resize group ${
            isDark ? 'hover:bg-gray-500' : 'hover:bg-gray-600'
          } ${isResizing ? (isDark ? 'bg-gray-500' : 'bg-gray-600') : ''}`}
          style={{ height: style?.height || '600px' }}
        >
              ...
              {selectedVisaData.officialUrl && (
                <a
                  ...
                  className={`block p-3 border text-center text-xs font-medium transition ${
                    isDark
                      ? 'border-gray-800 bg-gray-900/30 text-gray-400 hover:bg-gray-900/50'
              ...
              {hasVisaDataFromDB ? (
                <>
                      ...
                      {selectedCountryData.visaCategories.map((visa) => (
                        <button
                          ...
                          className={`px-3 py-1.5 text-xs font-medium border transition-colors cursor-pointer ${
                            isDark
                              ? 'border-gray-700 bg-gray-800 text-gray-300 hover:bg-gray-700 hover:border-gray-500'
                              : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-500'
                          }`}
                        >
                          {visa}
                        </button>
                  ...
                  <div className={`p-3 border-l-2 ${
                    isDark ? 'border-gray-500 bg-gray-950/30' : 'border-gray-500 bg-gray-50'
                  }`}>
                    <div className="flex items-start gap-2">
                      <svg className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isDark ? 'text-gray-400' : 'text-gray-600'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <div>
                        <div className={`text-xs font-medium mb-1 ${isDark ? 'text-gray-400' : 'text-gray-900'}`}>
                          Relocation Assistant
                        </div>
                        <p className={`text-xs leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-800'}`}>
